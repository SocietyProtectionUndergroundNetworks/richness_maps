...
projectFolder = '000_SPUN_GFv4_11/' + guild + '_singlemodel'
...
classDfSortedRegression.to_csv('output/'+today+'_'+classProperty+'_grid_search_results_regression_singlemodel.csv', index=False)
...
predObs_df.to_csv('output/'+today+'_'+classProperty+'_pred_obs_singlemodel.csv', index=False)
...
def finalImageClassification(compositeImg):
    ...
    if ensemble == True:
        def classifyImage(classifier):
            modelNameRegression = classifier
        ...
        classifiedImage = classifiedImage_Regression
    return classifiedImage
...
ensemblePredictedImage = regressedImage
...
def bootstrapFunc(fc):
    ...
    trainedClassiferRegression = classifierToBootstrapRegression.train(fc, classProperty, covariateList)
...
if log_transform_classProperty == True:
    ...
    ensemblePredictedImage.exp().subtract(1).rename(classProperty+'_Ensemble_mean'),
    ...
    finalImageToExport = ee.Image.cat(
    ensemblePredictedImage.rename(classProperty+'_Ensemble_mean'),
