...
dropped_stats <- df %>%
  ...
  group_by(Resolve_Biome) %>%
  summarise(n = mean(n), median = mean(median), iqr = mean(iqr), cutoff = mean(cutoff), n_dropped = sum(dropped), lower = mean(median - 5 * iqr), upper = mean(median + 5 * iqr))
...
# AMF
# Load data, rename biome names when writing per-biome summary file. Uncomment to retain integers (necessary for mapping)
df <- fread("/Users/johanvandenhoogen/SPUN/richness_maps/data/20250115_AMF_richness_rarefied_sampled.csv") %>% 
  mutate(Resolve_Biome = as.integer(Resolve_Biome))#%>% 
  # mutate(Resolve_Biome = replace(Resolve_Biome, Resolve_Biome == 1, "Tropical Moist Forests")) %>%
  # mutate(Resolve_Biome = replace(Resolve_Biome, Resolve_Biome == 2, "Tropical Dry Forests")) %>%
  # mutate(Resolve_Biome = replace(Resolve_Biome, Resolve_Biome == 3, "Tropical Coniferous Forests")) %>%
...
# Write to file
# fwrite(dropped_stats, '/Users/johanvandenhoogen/SPUN/richness_maps/output/20250115_AMF_SSU_outlier_removal_stats.csv')
...
# Write to file
fwrite(filtered_data, '/Users/johanvandenhoogen/SPUN/richness_maps/data/20250115_AMF_sampled_outliersRemoved.csv')
