...
# Skip pixel aggregation
#     Pixel aggregation; select variables to include
...
# preppedCollection = rawPointCollection.sample(frac=1).reset_index(drop=True)[covariateList+["Resolve_Biome"]+[classProperty]+['Pixel_Lat', 'Pixel_Long']]
...
if log_transform_classProperty == True:
    preppedCollection[classProperty] = np.log(preppedCollection[classProperty])
...
#     grid_search_results = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'grid_search_results_woAggregation')
    ...
    classDfSorted.to_csv('output/'+classProperty+'_grid_search_results_tmp.csv', index=False)

    # Format the bash call to upload the file to the Google Cloud Storage bucket
    gsutilBashUploadList = [bashFunctionGSUtil]+arglist_preGSUtilUploadFile+['output/'+classProperty+'_grid_search_results_tmp.csv']+[formattedBucketOI]
    subprocess.run(gsutilBashUploadList)
    ...
    # Upload the file into Earth Engine as a table asset
    assetIdForGridSearchResults = 'users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'grid_search_results_woAggregation'
    earthEngineUploadTableCommands = [bashFunction_EarthEngine]+arglist_preEEUploadTable+[assetIDStringPrefix+assetIdForGridSearchResults]+[formattedBucketOI+'/'+classProperty+'_grid_search_results_tmp.csv']+arglist_postEEUploadTable
    subprocess.run(earthEngineUploadTableCommands)
    print('Upload to EE queued!')
...
predObs = finalImageClassification(compositeToClassify).addBands(ee.Image.pixelLonLat().rename(['Pixel_Long', 'Pixel_Lat'])).sampleRegions(
	collection = fcOI,
...
predObsexport = ee.batch.Export.table.toAsset(
	...
	description = classProperty+'_predicted_observed',
	assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'predicted_observed'
)
predObsexport.start()
...
predObs_df.to_csv('output/20220410_'+classProperty+'_pred_obs.csv')
