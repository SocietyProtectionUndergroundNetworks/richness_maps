...
def pipeline(setup):
	...
	projectFolder = '000_SPUN_temp_log_zeroInflated'
	...
	'EsaCci_BurntAreasClassification',
	...
	try:
		...
		classDfSortedClassification.to_csv('output/'+classProperty+setup+'_grid_search_results_Classification.csv', index=False)
		...
		gsutilBashUploadList = [bashFunctionGSUtil]+arglist_preGSUtilUploadFile+['output/'+classProperty+setup+'_grid_search_results_Classification.csv']+[formattedBucketOI]
		subprocess.run(gsutilBashUploadList)
		print('grid_search_results'+' uploaded to a GCSB!')
	...
	# Write to file
	if log_transform_classProperty == True:
		predObs_df.to_csv('output/20220504_'+classProperty+'_pred_obs_zeroInflated_'+setup+'_logTransformed.csv')

	if log_transform_classProperty == False:
		predObs_df.to_csv('output/20220504_'+classProperty+'_pred_obs_zeroInflated_'+setup+'_wo_logTransformed.csv')
