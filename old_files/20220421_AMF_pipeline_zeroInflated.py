...
def pipeline(setup):
    ...
    if distinctObs == True:
        preppedCollection = fcToAggregate.drop_duplicates(subset = covariateList+[classProperty], keep = False)[covariateList+["Resolve_Biome"]+[classProperty]+['Pixel_Lat', 'Pixel_Long']]

    if pixel_agg == False and distinctObs == False:
        preppedCollection = fcToAggregate[covariateList+["Resolve_Biome"]+[classProperty]+['Pixel_Lat', 'Pixel_Long']]
    ...
    try:
        ...
        for rf in classifierListProbability:
            ...
            fcOI = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+titleOfCSVWithCVAssignments)
            fcOI = fcOI.map(lambda f: f.set('AMF_diversity', ee.Number(f.get('AMF_diversity')).divide(f.get('AMF_diversity')))) # train classifier on 0 (classProperty == 0) or 1 (classProperty != 0)
            # fcOI = fcOI.limit(100) # ofr testing only!
...
classifiedImageProbability.sampleRegions(
	collection = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+titleOfCSVWithCVAssignments).filter(ee.Filter.eq('AMF_diversity', 0)),
