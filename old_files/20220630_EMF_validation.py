...
titleOfCSVWithCVAssignments = '20220630_ECM_validationdata'
...
predObsexport = ee.batch.Export.table.toAsset(
	...
	description = '20220630_ECM_validation',
	assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+'20220630_ECM_validation'
)
predObsexport.start()
...
while count >= 1:
	taskList = [str(i) for i in ee.batch.Task.list()]
	subsetList = [s for s in taskList if '20220630_ECM_validation' in s]
	count = len(subsubList)
	print(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'), 'Waiting for pred/obs to complete...', end = '\r')
	time.sleep(normalWaitTime)
print('Moving on...')

predObs_wResiduals = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+'20220630_ECM_validation')
...
# Write to file
if log_transform_classProperty == True:
	predObs_df.to_csv('output/20220630_ECM_validation.csv')
