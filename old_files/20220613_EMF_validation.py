...
titleOfCSVWithCVAssignments = '20220613_ECM_tedersoo_validationdata'
...
rawPointCollection['sequencing_platform'] = int(GF_data[GF_data['sample_id'] == 'FMS25266v2']['target_marker'])
rawPointCollection['sample_type'] = int(GF_data[GF_data['sample_id'] == 'FMS25266v2']['sequencing_platform'])
rawPointCollection['primers'] = int(GF_data[GF_data['sample_id'] == 'FMS25266v2']['sample_type'])
rawPointCollection['target_marker'] = int(GF_data[GF_data['sample_id'] == 'FMS25266v2']['primers'])
...
fcOI_train = ee.FeatureCollection('users/johanvandenhoogen/000_SPUN_zeroInflated/ECM_diversitydistictObs_wProjectVars_wCV_folds_data')
	...
	if ensemble == True:
		def classifyFC(classifiers):
			...
			def classifyFunction(f):
				...
				# Calculate final predicted value as product of classification and regression
				featureToReturn = featureToReturn.set(classProperty+'_Predicted', ee.Number(f.get(classProperty+'_Classified')).multiply(ee.Number(f.get(classProperty+'_Regressed'))))
...
predObsexport = ee.batch.Export.table.toAsset(
	...
	description = '20220613_ECM_validation_wTedersoo',
	assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+'20220613_ECM_validation_wTedersoo'
)
predObsexport.start()
...
while count >= 1:
	taskList = [str(i) for i in ee.batch.Task.list()]
	subsetList = [s for s in taskList if '20220613_ECM_validation_wTedersoo' in s]
	count = len(subsubList)
	print(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'), 'Waiting for pred/obs to complete...', end = '\r')
	time.sleep(normalWaitTime)
print('Moving on...')

predObs_wResiduals = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+'20220613_ECM_validation_wTedersoo')
...
# Write to file
if log_transform_classProperty == True:
	predObs_df.to_csv('output/20220613_ECM_validation_wTedersoo.csv')
