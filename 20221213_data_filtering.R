...
summary_woBissetYan <- df %>% 
  ...
  summarise(n = n(), median = median(myco_diversity), iqr = IQR(myco_diversity)) %>% 
...
dropped_stats <- df %>%
  ...
  mutate(dropped = myco_diversity > cutoff) %>%
  group_by(Resolve_Biome, guild) %>%
  summarise(n = mean(n), median = mean(median), iqr = mean(iqr), cutoff = mean(cutoff), n_dropped = sum(dropped))

fwrite(dropped_stats, '/Users/johanvandenhoogen/SPUN/richness_maps/output/20221219_GFv4_outlier_removal_stats.csv')
