...
def pipeline(guild):
	...
	projectFolder = '000_SPUN_GFv4_3/' + guild
	...
	####################################################################################################################################################################
	# Import raw data
	rawPointCollection = pd.read_csv('data/20221122_GFv4_richness_rarefied_sampled.csv', float_precision='round_trip')
	...
	print('Filtered by guild', rawPointCollection.shape[0])
	...
	try:
		...
		if distinctObs == True:
			...
			preppedCollection = fcToAggregate.drop_duplicates(subset = covariateList+[classProperty], keep = 'first')[['sample_id']+covariateList+["Resolve_Biome"]+[classProperty]+['Pixel_Lat', 'Pixel_Long']+['source']]
...
if __name__ == '__main__':
		...
		with poolcontext(number_of_processes) as pool:
				...
				results = pool.map(pipeline, guildList)
