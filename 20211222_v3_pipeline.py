...
def pipeline(taxon):
    ...
    projectFolder = '000_SPUN/v3_combined/'+taxon
    ...
    titleOfCSVWithCVAssignments = taxon+"_wCV_folds_data"
    ...
    ####################################################################################################################################################################
    # Import the raw CSVs
    rawPointCollection_tedersoo = pd.read_csv('data/20211222_all_taxa_tedersoo_'+taxon+'_wMeta.csv', float_precision='round_trip')

    if taxon == 'ArbuscularMycorrhizal':
        globalfungistring = 'AMF'
    else:
        globalfungistring = 'ECM'
    rawPointCollection_globalFungi = pd.read_csv('/Users/johanvandenhoogen/SPUN/richness_maps/data/20211026_'+globalfungistring+'_diversity_data_sampled.csv')

    # Merge files
    rawPointCollection = rawPointCollection_tedersoo.append(rawPointCollection_globalFungi, ignore_index = True)

    # Rename columnto be mapped
    rawPointCollection.rename(columns={'richness': classProperty}, inplace=True)
    ...
    GEE_FC_to_pd(grid_search_results.limit(10, 'Mean_R2', False)).to_csv('output/'+taxon+'_grid_search_results.csv')
    ...
    imgExport = ee.batch.Export.image.toAsset(
        ...
        description = taxon+'predicted',
        assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+taxon+'_predicted' ,
        crs = 'EPSG:4326',
    ...
    print(taxon, 'Image export started')
