...
projectFolder = '000_SPUN_cleaned_data'
...
try:
    assetIDToCreate_Folder = 'projects/crowtherlab/johan/'+projectFolder
    ee.data.createAsset({'type': 'Folder'}, assetIDToCreate_Folder, False)
...
def data_processing():
	# Import raw data
	rawPointCollection = pd.read_csv('data/20220805_ECM_diversity_data_sampled_cleaned.csv', float_precision='round_trip')
	...
	# List of primers to remove; failed validation
	primers_toRemove = ["58A2F/ITS4",
					    "gITS7/ITS4ngs",
	...
	try:
		...
		fcOI = ee.FeatureCollection('projects/crowtherlab/'+usernameFolderString+'/'+projectFolder+'/'+titleOfCSVWithCVAssignments)
	    ...
	    # Remove duplicates or pixel aggregate; depending on setup
	    if pixel_agg == True:
	        # Pixel aggregate; mean per pixel
	    ...
	    if distinctObs == True:
	        # Distinct observations; unique values only
	    ...
	    if pixel_agg == False and distinctObs == False:
	        # No duplicate removal or pixel aggregation
	    ...
	    localPathToCVAssignedData = 'data/preppedCollection.csv'
	    preppedCollection.to_csv(localPathToCVAssignedData,index=False)

	    # !ls /datasets/google-cloud-bucket
	    preppedCollection.to_csv('/datasets/google-cloud-bucket/preppedCollection.csv',index=False)
	    # Upload the file into Earth Engine as a table asset
	    !earthengine --service_account_file './johan_service_account.json' upload table --asset_id=projects/crowtherlab/johan/000_SPUN_cleaned_data/ECM_diversitydistictObs_wProjectVars_wCV_folds_data gs://johanvandenhoogen/preppedCollection.csv --x_column Pixel_Long --y_column Pixel_Lat
...
def grid_search(fcOI):
	...
	# Try to look up previously finished grid search results
	try:
	...
	# If grid search was not performed yet:
		...
		try:
			...
			# Create list of finished models
			finished_models_classification = list(classDfClassification['cName'])
        ...
        classDfSortedRegression.to_csv('/datasets/google-cloud-bucket/'+classProperty+setup+'_grid_search_results_Regression_zeroInflated.csv',index=False)
        classDfSortedClassification.to_csv('/datasets/google-cloud-bucket/'+classProperty+setup+'_grid_search_results_Classification_zeroInflated.csv', index=False)

        # Upload the file into Earth Engine as a table asset
        !earthengine --service_account_file './johan_service_account.json' upload table --asset_id=projects/crowtherlab/johan/000_SPUN_cleaned_data/ECM_diversitydistictObs_wProjectVars_grid_search_results_Regression_zeroInflated gs://johanvandenhoogen/ECM_diversitydistictObs_wProjectVars_grid_search_results_Regression_zeroInflated.csv --x_column 'Pixel_Long' --y_column 'Pixel_Lat'
        !earthengine --service_account_file './johan_service_account.json' upload table --asset_id=projects/crowtherlab/johan/000_SPUN_cleaned_data/ECM_diversitydistictObs_wProjectVars_grid_search_results_Classification_zeroInflated gs://johanvandenhoogen/ECM_diversitydistictObs_wProjectVars_grid_search_results_Classification_zeroInflated.csv --x_column 'Pixel_Long' --y_column 'Pixel_Lat'
	...
	GEE_FC_to_pd(grid_search_resultsRegression.limit(10, 'Mean_R2', False)).to_csv('output/'+classProperty+'_grid_search_results_Regression.csv')
	GEE_FC_to_pd(grid_search_resultsClassification.limit(10, 'Mean_R2', False)).to_csv('output/'+classProperty+'_grid_search_results_Classification.csv')
...
def pred_obs(fcOI):
	try:
		...
		predObsexport = ee.batch.Export.table.toAsset(
			...
			assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'_pred_obs_zeroInflated_'+setup
		)
		predObsexport.start()
		...
		predObs_wResiduals = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'_pred_obs_zeroInflated_'+setup)
	...
	predObs_df.to_csv('output/20220727_'+classProperty+'_pred_obs_zeroInflated_'+setup+'.csv')
...
buffer_sizes = [5000, 10000, 50000, 100000, 250000, 500000, 750000, 1000000]
...
for buffer in buffer_sizes:
	...
	bloo_cv_fc_export = ee.batch.Export.table.toAsset(
		...
		assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'_sloo_cv_results_woExtrapolation_'+str(buffer)
	)
