...
# Arbuscular
df <- fread('output/20230510_arbuscular_mycorrhizal_richness_pred_obs.csv')
...
# Ecto
df <- fread('output/20230531_ectomycorrhizal_richness_pred_obs.csv') %>% 
  group_by(sample_id) 
...
# Arbuscular
df <- fread('output/20230502_arbuscular_mycorrhizal_richness_pred_obs_wMEM.csv')
...
df %>% 
  ...
  theme(legend.position = "none",
        aspect.ratio = 1) +
  labs(y = "Predicted AMF Richness", x = "Observed AMF Richness", title = "with MEMs") 

# Ecto
df <- fread('output/20230531_ectomycorrhizal_wMEM_richness_pred_obs.csv') %>% 
  group_by(sample_id) 
...
df %>% 
  ...
  theme(legend.position = "none",
        aspect.ratio = 1) +
  labs(y = "Predicted ECM Richness", x = "Observed ECM Richness", title = "with MEMs") 
