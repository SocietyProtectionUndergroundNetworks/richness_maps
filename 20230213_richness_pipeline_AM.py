...
projectFolder = '000_SPUN_GFv4_8/' + guild + '_woProject'
...
try:
    ...
    def predObs(fcOI):
        if ensemble == False:
            ...
            fcOIforClassification = fcOI.map(lambda f: f.set('sequencing_platform', 1))
            fcOIforClassification = fcOIforClassification.map(lambda f: f.set('sample_type', 0))
            fcOIforClassification = fcOIforClassification.map(lambda f: f.set('primers', 4))
            # Classify the FC
