...
setup = 'wopixelAgg_woProjectVars'
...
projectFolder = '000_SPUN_spatial_preds'
...
# Spatial predictor layers (produced by Oleh)
spatial_predictors = ee.Image('users/johan_spun/ECM_spatial_predictors_img')

# Composite image with env layers
compositeOfInterest = ee.Image('projects/crowtherlab/Composite/CrowtherLab_Composite_30ArcSec').addBands(spatial_predictors)
...
outputFolder = '/Users/johanvandenhoogen/SPUN/richness_maps/output/spatial/'
...
####################################################################################################################################################################
# Import raw data
rawPointCollection = pd.read_csv('data/20220805_ECM_diversity_log_pixelAggMedian_wSpatialPreds.csv', float_precision='round_trip')
...
try:
	...
	if pixel_agg == False and distinctObs == False:
		...
		preppedCollection = fcToAggregate[covariateList+["Resolve_Biome"]+[classProperty]+['Pixel_Lat', 'Pixel_Long']+['source']]
...
try:
	...
	try:
		...
		classDfRegression = pd.read_csv('output/'+classProperty+setup+'_grid_search_results_Regression_spatial.csv')
	...
	for rf in classifierListRegression:
		if rf.get('cName').getInfo() in finished_models_regression:
			...
			classDfRegression.to_csv(outputFolder+classProperty+setup+'_grid_search_results_Regression_spatial.csv', index=False)

	finished_models_classification = list()
	try:
		# Check if any models have been completed
		classDfClassification = pd.read_csv('output/'+classProperty+setup+'_grid_search_results_Classification_spatial.csv')
	...
	for rf in classifierListClassification:
		if rf.get('cName').getInfo() in finished_models_classification:
			...
			classDfClassification.to_csv(outputFolder+classProperty+setup+'_grid_search_results_Classification_spatial.csv', index=False)
	...
	classDfSortedRegression.to_csv(outputFolder+classProperty+setup+'_grid_search_results_Regression_spatial.csv', index=False)
	classDfSortedClassification.to_csv(outputFolder+classProperty+setup+'_grid_search_results_Classification_spatial.csv', index=False)

	# Format the bash call to upload the file to the Google Cloud Storage bucket
	gsutilBashUploadList = [bashFunctionGSUtil]+arglist_preGSUtilUploadFile+[outputFolder+classProperty+setup+'_grid_search_results_Regression_spatial.csv']+[formattedBucketOI]
	subprocess.run(gsutilBashUploadList)
	print('grid_search_results'+' uploaded to a GCSB!')

	gsutilBashUploadList = [bashFunctionGSUtil]+arglist_preGSUtilUploadFile+[outputFolder+classProperty+setup+'_grid_search_results_Classification_spatial.csv']+[formattedBucketOI]
	subprocess.run(gsutilBashUploadList)
	print('grid_search_results'+' uploaded to a GCSB!')
	...
	earthEngineUploadTableCommands = [bashFunction_EarthEngine]+arglist_preEEUploadTable+[assetIDStringPrefix+assetIdForGridSearchResults]+[formattedBucketOI+'/'+classProperty+setup+'_grid_search_results_Regression_spatial.csv']+arglist_postEEUploadTable
	subprocess.run(earthEngineUploadTableCommands)
	...
	earthEngineUploadTableCommands = [bashFunction_EarthEngine]+arglist_preEEUploadTable+[assetIDStringPrefix+assetIdForGridSearchResults]+[formattedBucketOI+'/'+classProperty+setup+'_grid_search_results_Classification_spatial.csv']+arglist_postEEUploadTable
	subprocess.run(earthEngineUploadTableCommands)
	print('Upload to EE queued!')
...
try:
	predObs_wResiduals = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'_pred_obs_zeroInflated_'+setup+'_logTransformed2')
...
predObs_df.to_csv('output/spatial/20220815_'+classProperty+'_pred_obs_spatial.csv')
...
interpolatedResidualsImg = predObs_wResiduals.inverseDistance(range = 1000000,
...
imgExport = ee.batch.Export.image.toAsset(
    ...
    assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'interpolatedResidualsImg_1000km' ,
    crs = 'EPSG:4326',
