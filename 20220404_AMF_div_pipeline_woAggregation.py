...
#     grid_search_results = ee.FeatureCollection('users/'+usernameFolderString+'/'+projectFolder+'/'+classProperty+'grid_search_results_woAggregation')
    ...
    modelResults = pd.DataFrame({'classProperty': classProperty,
                                 'cName': bestModelName,
    ...
    with open('output/'+classProperty+'_grid_search_results.csv', 'a') as f:
        modelResults.to_csv(f, mode='a', header=f.tell()==0)

    # Format the bash call to upload the file to the Google Cloud Storage bucket
    gsutilBashUploadList = [bashFunctionGSUtil]+arglist_preGSUtilUploadFile+['output/'+classProperty+'_grid_search_results.csv']+[formattedBucketOI]
    subprocess.run(gsutilBashUploadList)
    print(titleOfCSVWithCVAssignments+' uploaded to a GCSB!')
    ...
    earthEngineUploadTableCommands = [bashFunction_EarthEngine]+arglist_preEEUploadTable+[assetIDStringPrefix+assetIdForGridSearchResults]+[formattedBucketOI+'/'+titleOfCSVWithCVAssignments+'.csv']+arglist_postEEUploadTable
    subprocess.run(earthEngineUploadTableCommands)
    print('Upload to EE queued!')
...
predObs = pred_climate_current.addBands(ee.Image.pixelLonLat().rename(['Pixel_Long', 'Pixel_Lat'])).sampleRegions(
	collection = fcOI,
