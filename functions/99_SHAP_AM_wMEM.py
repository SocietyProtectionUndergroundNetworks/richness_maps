...
df = pd.read_csv('data/arbuscular_mycorrhizal_richness_training_data_wMEMs.csv')
...
# Spatial predictors
AM_spatial_ic = ee.ImageCollection('users/johanvandenhoogen/000_SPUN/MEM_AMF')
AM_spatial = AM_spatial_ic.toBands().rename(AM_spatial_ic.aggregate_array('system:index'))

# Get bandNames of spatial predictors
spatial_preds = AM_spatial_ic.aggregate_array('system:index').getInfo()
                
# Create list of all covariates
covariateList = envCovariateListRenamed + project_vars + spatial_preds
...
if __name__ == '__main__':
    ...
    with poolcontext(NPROC) as pool:
        ...
        np.savez('shap_values_AM_MEM.npz', *shap_values_list)

        # Read SHAP values from file
        with np.load('shap_values_AM_MEM.npz') as data:
        ...
        plt.tight_layout()
        plt.savefig('figures/20230731_arbuscular_mycorrhizal_richness_shap_summary_plots_full_wMEM.png', dpi=300)
        ...
        # Plot 3: SHAP summary plot, with project_vars and MEMs grouped together
        # Sum project_vars SHAP values
        project_shap_values = np.sum(mean_shap_values[:, len(covariateList) - len(project_vars) - len(spatial_preds):len(covariateList) - len(spatial_preds):], axis=1).reshape(-1, 1)

        # check if right vars are selected
        covariateList[len(covariateList) - len(project_vars) - len(spatial_preds):len(covariateList) - len(spatial_preds)] == project_vars

        # Sum MEMs SHAP values
        MEM_shap_values = np.sum(mean_shap_values[:, len(covariateList) - len(spatial_preds):], axis=1).reshape(-1, 1)

        # check if right vars are selected
        covariateList[len(covariateList) - len(spatial_preds):] == spatial_preds

        # Get SHAP values for other features
        other_shap_values = mean_shap_values[:, :len(covariateList) - len(project_vars) - len(spatial_preds)]

        # Combine 'project_vars' and 'MEM' SHAP values with other features
        combined_shap_values = np.hstack([other_shap_values, project_shap_values, MEM_shap_values])

        # Create new feature names list
        new_feature_names = envCovariateListRenamed + ["Project Variables"] + ["MEMs"]
        ...
        df_project_vars_grouped.loc[:, 'MEMs'] = np.NaN

        plt.figure()
        shap.summary_plot(combined_shap_values, features = df_project_vars_grouped, sort=True, show = False)
        plt.xlabel('Mean absolute SHAP value')
        plt.tight_layout()
        plt.savefig('figures/20230731_arbuscular_mycorrhizal_richness_shap_summary_plots_projectGrouped_wMEM.png', dpi=300)
