...
# footprint = ee.Image('users/johanvandenhoogen/000_SPUN_GFv4_9/ectomycorrhizal/ectomycorrhizal_richness_Classified_MultibandImage').select(0)
# intext = ee.Image('users/johanvandenhoogen/000_SPUN_GFv4_9/ectomycorrhizal/ectomycorrhizal_richness_Classified_MultibandImage').select('PCA_pct_int_ext')
# intext = intext.updateMask(footprint)
footprint = ee.Image('users/johanvandenhoogen/000_SPUN_GFv4_10/arbuscular_mycorrhizal/arbuscular_mycorrhizal_richness_Classified_MultibandImage').select(0)
intext = ee.Image('users/johanvandenhoogen/000_SPUN_GFv4_10/arbuscular_mycorrhizal/arbuscular_mycorrhizal_richness_Classified_MultibandImage').select('PCA_pct_int_ext')
intext = intext.updateMask(footprint)

# Composite image to sample
compositeToUse = ee.Image.cat([
		...
		]).select(['Pixel_Lat', 'Pixel_Long']).updateMask(intext.gte(0.95))
...
if __name__ == '__main__':
		...
		with poolcontext(NPROC) as pool:
				...
				# results.to_csv("data/filtered_randomPoints_ECM.csv", index=False)
				results.to_csv("data/filtered_randomPoints_AMF.csv", index=False)