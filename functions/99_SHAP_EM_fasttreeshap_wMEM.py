...
for cat in project_vars + spatial_preds:
...
def calculate_shap_values(rep):
    grid_search_results = pd.read_csv('output/20230731_ectomycorrhizal_richness_grid_search_results_Regression_wMEM.csv')
...
if __name__ == '__main__':
    ...
    with poolcontext(NPROC) as pool:
        ...
        # np.savez('shap_values_EM_MEM_fasttree.npz', *shap_values_list)

        # Read SHAP values from file
        with np.load('shap_values_EM_MEM_fasttree.npz') as data:
        ...
        plt.tight_layout()
        plt.savefig('figures/20230731_ectomycorrhizal_richness_shap_summary_plots_full_wMEM.png', dpi=300)
        ...
        plt.xlabel('Mean absolute SHAP value')
        plt.tight_layout()
        plt.savefig('figures/20230731_ectomycorrhizal_richness_shap_summary_plots_projectGrouped_wMEM.png', dpi=300)
