...
def pipeline(taxon):
    ...
    projectFolder = '000_SPUN/tedersoo/'+taxon
    ...
    ####################################################################################################################################################################
    # Import the raw CSV
    rawPointCollection = pd.read_csv('data/20211206_all_taxa_tedersoo_'+taxon+'.csv', float_precision='round_trip')
    ...
    pred_climate_current = staticCompositeImg.addBands(climate_2015).select(covariateList).reproject(staticCompositeImg.projection())
    ...
    # 'pred_futureClimate_rcp85_2050',
    # 'pred_futureClimate_rcp85_2070'])
    image_toExport = ee.ImageCollection(list(map(finalImageClassification, [pred_climate_current]))).toBands().rename(['pred_climate_current'])
    imgExport = ee.batch.Export.image.toAsset(
        image = image_toExport.toFloat(),
        description = taxon+'_predicted',
        assetId = 'users/'+usernameFolderString+'/'+projectFolder+'/'+taxon+'_init_predicted' ,
        crs = 'EPSG:4326',
